					...
					<ul>
						...
						<c:forEach var="i" begin="${5*(index-1)+1}" end="${5*(index-1)+5}">
							<c:if test="${i<=totalpage}">
								<c:if test="${i eq page}">
									<li class="selected"><a href="${pageContext.request.contextPath }/board?page=${i}&index=${index}">${i}</a></li>
								</c:if>
								<c:if test="${i ne page}">
									<li ><a href="${pageContext.request.contextPath }/board?page=${i}&index=${index}">${i}</a></li>
								</c:if>
							</c:if>
						</c:forEach>
